...
To re-center your screen, use the **Recenter display** button in the plugin sidebar, perform a double-tap on your glasses, or enable **Recenter with a controller combo** in the sidebar to trigger it by holding a controller button combo (works even mid-game, when Steam Input has grabbed the controller).
